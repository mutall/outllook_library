...
//2 minutes from now
export function once(file:string):void{
    //
...
export async function cancel():Promise<void>{
    ...
    const Plan:mutall_user.plan = await mutall_user.event.seek(pk);
    //
    //Cancel the plan
...
export async function repetive(file:string):Promise<void>{
    //
