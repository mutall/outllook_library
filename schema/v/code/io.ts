...
export type io_type =
    ...
    //of a text area is accessec via the textContent property
    |'textarea'
    //
...
export interface anchor {
    ...
    element:HTMLElement,
    //
    //The view in which this io resides. This is important for navigating outside
    //of the page where the io is located. We need to be able to restore this page
    //when we come back. For instance, editing a foreign key from an explorer
    //page launces a new explorer page. The new page requires this base as its 
    //mother
    page: view.page
}
...
export abstract class io extends view.view{
    ...
    static create_io(
        ...
        const result:io_type|io = io.get_io_type(anchor, type, col);
        //
    ...
    static get_io_type(anchor:anchor, type?:io_type, col?:schema.column):io_type|io{
        //
        //If the type is defined, return it
        if (type!==undefined) return type;
        //
        //If the database column is known, deduce the io type from it
        if (col!==undefined) return io.deduce_io(anchor, col);
        //
    ...
    static deduce_io(
        ...
        if (col.length! > 100) return new textarea(anchor,col);
        //
        //If the column name is 'description', then its a text area
        if (col.name === 'description')  return new textarea(anchor, col);
        //
    ...
    get name(): string|undefined{
        ...
        if (this.col===undefined) return undefined;
        //
...
export class url extends io{
    ...
    // 
    // 
    constructor(anchor:anchor, col?:schema.column) {
        super(anchor, col);
        //
...
export class textarea extends input{
    ...
    //
    constructor(anchor:anchor, col?:schema.column, size?:number) {
        //
...
export class primary extends io{
    ...
    //
    constructor(anchor:anchor, public col:schema.column){
        super(anchor, col);
        //
