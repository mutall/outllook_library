...
export function send(){
    ...
    const report:HTMLSpanElement|null = document.getElementById('result');
    //
    //You may have failed to define it in your form
    if (report===null) throw new schema.mutall_error('Result tag not found');
    //
    ...
    const result:'ok'|Error = msg.send([emailer]);
    //
    //Compile the resulting response
    const response: string = result === 'ok' ? 'ok' : result.message;
    //
    //Show the response
