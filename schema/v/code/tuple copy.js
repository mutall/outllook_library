...
//methods that are shared by all entities, e.g., save()
//
...
//
//Reference database-specific classes that are auto-generated by the ORM-mapper 
import * as mutall_user from "./mutall_user.js";
//
//The list of databases on a server is a dictionary of databases indexed by 
//their names
const databases = {
    //
    mutall_user: mutall_user
};
//The pointer class allows us to manage lists of tuples of some tyoe. 
export class pointer {
    ...
    get col() {
        //
        //Get the name of teh member constructor
    ...
    //if x.members===undefined awwit x.populate(); x.map(...)
    //
    ...
    async populate() {
        ...
        if (this.members !== undefined)
            return this.members;
        //
        //Continue only if the away pk is valid
        ...
        //
        //Get the home database name of the foreign key for this pointer
        ...
        const factory = databases[dbname][ename];
        //
...
//The tuple class is the root of all classes derived automaticaly by mapping the 
//relation to the object model. It extends the schema entity. The raedy parameter
//allows us to work with the ready feed without the need for casting.
export class tuple {
    dbname;
    ename;
    feed;
    ...
    pointers = {};
    //
    //A tuple has an entity associated with it.
    entity;
    //
    //Initialize this tuple with the relevant entity schema
    constructor(
    //
    //The tuple's database
    dbname, 
    //
    //The entity name
    ename, 
    //
    //The tuple's input data feed; it is undefined for brand new tuples
    feed) {
        this.dbname = dbname;
        this.ename = ename;
        this.feed = feed;
        //Set the tuple's entity (from the databases defiend in the schema 
        //namespace). NB. The databases defined in the tuple and schema namespaces
        //are very diferent. Here are the differences:-
        //schema.databases = {[dbname:schema.dbname]:schema.database}
        //tuple.databases = {[dbname:schema.dbname]:tuple.database}
        //A schema database has dictinary of all entities derievd from the 
        //informaton schema
        //A tuple database is a dictionary of concrete classes by ORMM mapping 
        this.entity = schema.databases[this.dbname].entities[ename];
        //
    ...
    async seek_scalars(value) {
        ...
        const result = await server.exec('editor', [this.ename, this.dbname], 'describe', []);
        //
        ...
        const records = await server.exec('database', [this.dbname], 'get_sql_data', [sql]);
        //
        ...
        for (const dbname in schema.databases) {
            ...
            for (const ename in dbase.entities) {
                ...
                for (const cname in dbase.entities[ename].columns) {
                    ...
                    if (col.ref.dbname !== this.dbname)
                        continue;
                    if (col.ref.ename !== this.ename)
                        continue;
                    //
                    //Collect this column
                    yield col;
